...
/* For this example:  Harmonic, Above LUB, Feasible, Zero-Margin (Unsafe)
...
LUB = 77.98%
LCM = 10     // period for repetition
U_tot = 100%

0ms    10ms   20ms   30ms   40     50     60    70      80     90    100 [ms]
|--S1--|--S2--|--S1--|--S2--|--S1--|--S2--|--S1--|--S2--|--S1--|--S3--|
...
#define PERIODS (10)       // how many periods execute
#define NUM_THREADS (3+1)  // 4 services + sequencer
#define TASK_DURATION 9.9  // time duration for the tasks
...
int main(void)
{
    ...
    {
       printf(" uname: %s\n", systemName.sysname);
       syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:5]: %s %s %s %s %s", systemName.sysname, systemName.nodename, systemName.release, systemName.version, systemName.machine);   
    }
...
void *Sequencer(void *threadp)
{
    ...
    do
    {
        ...
        // Service_3 = RT_MAX-3	@ 14 Hz
        if ((seqCnt % 10) == 0)  { sem_post(&semS3); printf(" S3"); }
...
void *Service_1(void *threadp)
{
    ...
    while(!abortS1)
    {
        ...
        syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:5]: Thread %d start %lld @%6.9lf on core 1\n", threadParams->threadIdx, S1Cnt, current_realtime-start_realtime);
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
        ...
        // work load
        fibonacciForXms(TASK_DURATION); // Service 2 has 2 units of computation time.
        fibonacciForXms(TASK_DURATION); 
    }
...
Dec 12 23:34:31 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Linux debian 4.19.0-16-amd64 #1 SMP Debian 4.19.181-1 (2021-03-19) x86_64
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 1 start 1 @0.000083977 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 2 start 1 @0.010307036 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 1 start 2 @0.021054669 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 3 start 1 @0.038800899 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 1 start 3 @0.050315663 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 2 start 2 @0.060390848 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 1 start 4 @0.070517046 on core 1
Dec 12 23:34:32 debian sequencer[3592]: [COURSE:2][ASSIGNMENT:5]: Thread 1 start 5 @0.097730493 on core 1
