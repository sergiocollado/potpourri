// Edited from the code from Sam Siewert
//
...
// Service_3 -  6 Hz ,every  15th Sequencer loop

//
// With the above, priorities by RM policy would be:
//
// Sequencer = RT_MAX	@ 100 Hz -> T = 10ms
// Servcie_1 = RT_MAX-1	@ 50  Hz -> T = 20ms
// Service_2 = RT_MAX-2	@ 10  Hz -> T = 100ms
// Service_3 = RT_MAX-3	@  6  Hz -> T = 150ms
...
#define _GNU_SOURCE  // This is necessary for CPU affinity macros in Linux
...
#define NUM_THREADS (3)  //redefine as needed 
...
// the STC that Raspbian presents as a clocksource. Based on the source code, â€œSTC: a free running counter
// that increments at the rate of 1MHzâ€. This means it increments every microsecond.
//
...
void main(void)
{
   ...
   printf("Using CPUS=%d from total available.\n", CPU_COUNT(&allcpuset));
    ...
    // assign the SIGALRM to the Sequencer function.
    signal(SIGALRM, (void(*)()) Sequencer);

    /* arm the interval timer */
    itime.it_interval.tv_sec = 0;
    itime.it_interval.tv_nsec = 10000000; //10+8 nsec = 10+5 usec = 10 ms -> 100 Hz 
    /* arm the retrigger timer interval */
...
void Sequencer(int id)
{
    ...
    printf("seqCnt=%llu\t", seqCnt); // actually we should use write(STDOUT_FILENO, because printf is not safe inside a signal handler function.
    syslog(LOG_CRIT, "seqCnt=%llu",seqCnt);
 

    // Release each service at a sub-rate of the generic sequencer rate


    // Servcie_1 = RT_MAX-1	@ 100/2 = 50 Hz
    if((seqCnt % 2) == 0) { sem_post(&semS1); printf(" S1"); }

    // Service_2 = RT_MAX-2	@ 100/10 = 10 Hz
    if((seqCnt % 10) == 0) { sem_post(&semS2); printf(" S2"); }

    // Service_3 = RT_MAX-3	@ 100/15 = 6 Hz
    ...
    if(abortTest || (seqCnt >= sequencePeriods))
    {
        ...
        timer_settime(timer_1, flags, &itime, &last_itime);
	printf("\nDisabling sequencer interval timer with abort=%d and %llu of %llu\n", abortTest, seqCnt, sequencePeriods);
    ...
    seqCnt++;  //increment the sequence number
}
