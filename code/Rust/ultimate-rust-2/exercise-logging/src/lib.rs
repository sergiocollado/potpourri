...
impl Frog {
    ...
    pub fn hop(&mut self) {
        self.energy -= 1;
        // 3. Use info!() to log that a Frog hopped, and how much energy is leftA
        info!("energy left {}", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("the frog ran out of energy! it will go to sleep");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!("Default frog was generated: {:?}", frog);
        frog
    }
}
