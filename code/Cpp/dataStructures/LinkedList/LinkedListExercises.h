...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // 1 - check if the list is empty. in this case the data should be at the front
  if (empty() == true) {
      pushFront(newData);
      //std::cout << "\n Sergio empty list " << newData << "\n";
  }
  // 2 - check if newData is smaller that the front of the list
  else if(newData <= front()) {
      pushFront(newData);
      //std::cout << "\nSergio Inserted at front(" << newData << ")\n";
  }
  // 3 - check if newData is bigger that the last element of the list
  else if(newData >= back()) {
      pushBack(newData);
      //std::cout << "\nSergio Inserted at back(" << newData << ")\n";
  }
  // 4 - if front() < newData < back()
  else {
      Node* prev = nullptr;
      Node* cur = head_;
      while (cur->next) {
	      ...
	      prev->next = newNode;
	      cur->prev  = newNode;
	      //std::cout << "\nSergio inserted in the middle: (" << newData << "), btween: (" << prev->data << ") and (" << cur->data << ")\n";
	      break;
	  }
      }
      size_++;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  if (left.empty() == true) { return right; }         // check one list is not emtpy
  else if (right.empty() == true ) { return left; }   // check the other list is not empty
  else {

      while (left.empty() == false || right.empty() == false) { //while the lists are not empty add elements.
             
	  if (left.empty() == true) {
             //std::cout << "\n Sergio : Left is empty\n";
	  ...
	  //std::cout << "Sergio : left.front = " << left.front() <<  "; right.front = " << right.front() <<  "\n";
	  if (left.front() <= right.front() ) {
              merged.pushBack(left.front());
	      //std::cout << "\n Sergio : Left element pushedFront(" << left.front() << ")\n"; 
              left.popFront();
	  }
	  else {
              merged.pushBack(right.front());
	      //std::cout << "\n Sergio : Right element pushedFront(" << right.front() << ")\n"; 
	      right.popFront();
	  }
      }
  }
