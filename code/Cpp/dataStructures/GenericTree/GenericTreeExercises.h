...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // ...
  //
  // 1- retrieve the tree root, and delete the whole tree
  //tree.deleteSubtree(tree.getRootPtr());
  tree.clear();
  // 2- build the requested tree.
  GenericTree<int>::TreeNode* node_4  = tree.createRoot(4);
  auto node_8  = tree.getRootPtr()->addChild(8);
  auto node_15 = tree.getRootPtr()->addChild(15);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  tree.compress(); //for safety reasons, we don't want nullptr dangling around.

  std::vector<TreeNode*> lastLevel; 
  std::vector<TreeNode*> nextLevel;

  lastLevel.push_back(rootNodePtr);

  do {
      for(TreeNode* node : lastLevel) {
          results.push_back(node->data);
          for(TreeNode* nodeChild : node->childrenPtrs) {
              nextLevel.push_back(nodeChild);
          }
      }
      lastLevel = std::move(nextLevel);
      nextLevel.clear();
