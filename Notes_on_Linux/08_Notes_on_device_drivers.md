...
**I2C (Inter-Integrated Circuit)** is a synchronous, multi-controller/multi-target, packet-switched, **single-ended, serial communication bus** developed by Philips in 1982. It is widely used to connect **low-speed peripherals to microcontrollers** or short-distance embedded systems over just two wires. 


#### Physical Layer & Hardware
The physical hardware relies on a unique electrical design:
* **Two Wires:** **SDA** (Serial Data) handles bidirectional data transfer, and **SCL** (Serial Clock) carries the clock signal generated by the master.
* **Open-Drain Topology:** Devices can only actively pull the lines low (0); they cannot drive them high (1).
* **Pull-Up Resistors:** External resistors (typically **4.7 kΩ**) pull the lines up to $V_{CC}$ when the bus is idle. This prevents short circuits if two devices transmit simultaneously.

---

#### Protocol & Message Frame
Data transfers occur in structured, **8-bit bytes (MSB first)**, with a 9th clock pulse dedicated to feedback:

| Bit Segment | Width | Purpose |
| :--- | :--- | :--- |
| **Start Condition** | 1 bit | Master pulls SDA low while SCL stays high. |
| **Address Frame** | 7 or 10 bits | Unique binary identifier for the target peripheral. |
| **Read/Write Bit** | 1 bit | Specifies direction: `0` for Write, `1` for Read. |
| **ACK/NACK Bit** | 1 bit | Receiver pulls SDA low to Acknowledge receipt. |
| **Data Frame** | 8 bits | Actual payload being moved across the bus. |
| **Stop Condition** | 1 bit | Master releases SDA high while SCL stays high. |


#### Key Features
* **Clock Stretching:** If a slow target peripheral needs time to process data, it can hold the SCL line low to temporarily pause the controller.
* **Arbitration:** Built-in hardware prevents collisions if multiple controllers attempt to take the bus at once, safely deciding a winner without damaging components.
* **Speed Modes:**
...
For more technical implementations, you can consult specific guides like the [TI I2C Basics Guide](https://ti.com) or the [Analog Devices I2C Primer](https://analog.com).

I2C protocol refresher: 
 - https://learn.sparkfun.com/tutorials/i2c/all
 - PDF - a basic guide to i2c: https://www.ti.com/lit/pdf/sbaa565
 - https://www.rocktech.com.hk/rocktech-blog/what-is-i2c-an-in-depth-look-at-the-linux-i2c-framework/
...
reference: https://medium.com/@tonyhe8688/mastering-i2c-driver-development-on-embedded-linux-89824179617e
 ...
 - **I2C Target (Client / Slave)**: The device you’re trying to access. Requires a device driver.
