...
// S: there are two global variables here
//  - the superblock structure
//  - the file descritor for the device
...
main(int argc, char **argv)
{
	...
	// S: firt thing is open the device,
	// most probably we should check also if it is in use.
	devfd = open(argv[1], O_RDWR);
	if (devfd < 0) {
		fprintf(stderr, "spfs-mkfs: Failed to open device\n");
		return(1);
	}
	// S: notice, the device is being open for reading & writting
	// .. so with fsdb, administratibe task can be done,
	// so the filesystem could be repaired in case of need.
	...
	// S: once readed the superblock, some basic information
	// about the filesystem is retrieved:
