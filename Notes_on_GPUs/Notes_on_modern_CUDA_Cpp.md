...
<img src="https://github.com/sergiocollado/potpourri/blob/master/Notes_on_GPUs/images/cuda_3_05/shared.png" alt= "Shared" width=900>
...
Our kernel performance is now at acceptable levels. However, we’ve made a significant oversight in our strategy. While writing host-side code, we used accelerated libraries. However, once we started writing CUDA kernels, we ended up implementing everything from scratch. This isn’t the best approach.
...
compile and run with:
```bash
!nvcc --extended-lambda -o /tmp/a.out Sources/block-sum.cu # build executable
!/tmp/a.out # run executable
```
...
You can compute the [block histogram using CUB](https://nvidia.github.io/cccl/cub/api/classcub_1_1BlockHistogram.html).
from the block size, number of bins, and elements per thread:
...
```bash
import Sources.dli
Sources.dli.run("Sources/cooperative.cu")
```
